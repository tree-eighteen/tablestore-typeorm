// src/typeorm-style/decorators/update-date-column.decorator.ts

import "reflect-metadata";
import { metadataStorage, ColumnMetadata } from './metadata-storage';

/**
 * UpdateDateColumn 装饰器选项
 */
export interface UpdateDateColumnOptions {
  /** 列名，如果不指定则使用属性名 */
  name?: string;
  /** 是否可为空 */
  nullable?: boolean;
 ...
 * 用于标记一个属性为更新时间列，会在实体创建和更新时自动设置当前时间
 * 
 * @param options 更新时间列选项
 * @returns 属性装饰器
 * 
 * @example
 * ```typescript
 ...
 *   updatedAt: Date;
 * 
 *   @UpdateDateColumn({ tablestoreType: 'STRING' })
 ...
 * ```
 */
export function UpdateDateColumn(options?: UpdateDateColumnOptions): PropertyDecorator {
  return function (target: any, propertyKey: string | symbol) {
    ...
    const columnMetadata: ColumnMetadata = {
      ...
      specialType: 'update-date',
      setOnInsert: true,  // 在插入时也设置
      setOnUpdate: true,  // 在更新时设置
      isPrimary: false
    };
    ...
    // 在属性上设置元数据标记
    Reflect.defineMetadata('tablestore:updateDateColumn', true, target, propertyKey);
    Reflect.defineMetadata('tablestore:column', true, target, propertyKey);
  };
}

/**
 * 检查一个属性是否被 @UpdateDateColumn 装饰器标记
 */
export function isUpdateDateColumn(target: any, propertyKey: string | symbol): boolean {
  return Reflect.getMetadata('tablestore:updateDateColumn', target, propertyKey) === true;
}
